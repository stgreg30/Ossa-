...
action is returned. A plasticity multiplier allows the
penalty sensitivity to be adjusted over time.
"""
...
class DecisionEngine:
    ...
    # Multiplier that can be dynamically tuned (e.g. by SynapticPlasticity)
    NEGATIVE_PENALTY_MULTIPLIER: float = 1.0
    ...
    def _score_outcome(self, outcome: str, positive_keywords: List[str]) -> int:
        ...
        Negative score for each penalised keyword found, scaled by the multiplier.
        ...
        return positive_score - int(penalty * self.NEGATIVE_PENALTY_MULTIPLIER)